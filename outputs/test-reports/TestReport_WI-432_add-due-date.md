...
-  TaskCard render test - renders the task title without crashing given valid props
-  TaskCard interaction test - calls onComplete with the task id when the Mark Complete button is clicked
-  TaskCard edge case - renders without crashing when optional props are absent
-  TaskCard renders DueDateBadge when task has a dueDate
-  TaskCard does not render DueDateBadge when the task is completed
-  TaskCard renders Completed badge when task is completed
