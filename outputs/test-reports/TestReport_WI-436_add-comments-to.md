...
-  CommentButton render test - renders a button with the comment count and correct aria-label
-  CommentButton interaction test - calls onClick when the button is clicked
-  CommentButton edge case - renders correctly when commentCount is 0
-  CommentPanel render test - renders the task title and textarea when activeTask is provided
-  CommentPanel render test - renders null when activeTask is null
-  CommentPanel interaction test - calls onClose when the close button is clicked
-  CommentPanel interaction test - Save button is disabled when input is empty
-  CommentPanel interaction test - shows validation error when text exceeds 500 characters
-  CommentPanel interaction test - calls postComment and onCommentAdded on successful save
-  CommentPanel interaction test - shows save error when postComment returns null
-  CommentPanel edge case - calls onClose when Escape key is pressed
-  CommentPanel edge case - does not render when activeTask is null and overlay is not present
